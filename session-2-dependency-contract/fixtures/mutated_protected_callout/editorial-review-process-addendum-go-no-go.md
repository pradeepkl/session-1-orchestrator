...
7. **Conversational tone** — Does the fix preserve the register established
   by `tone-reference.md` and `narrative-voice-directive.md` (plain
   declarative sentences, problem-before-abstraction, no self-referential
   "this chapter" framing in Summaries), or does it introduce hedge words,
   meta-commentary, or abstract framing that those documents already warn
   against?

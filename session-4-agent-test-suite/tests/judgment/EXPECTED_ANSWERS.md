...
**Justification:** Playbook opening: *"Anything about when a call happens, which pass this is, which file is current, or whether to stop is decided by the orchestrator… Do exactly the task named in the call… and return exactly the schema."* The agent performs EVALUATE judgment only; orchestration verbs in the comment are outside its role. **Ambiguity resolution (Pass 2 review):** This row was flagged as possibly redundant with `playbook.evaluate.no_apply_decision` and `playbook.apply.no_pass_or_filesystem_control`; ruling is KEEP as its own JUDGMENT scenario — this entry tests whether the agent **resists an external instruction** (orchestration verbs embedded in a review comment) to overstep its role, whereas those two rows test whether the agent **does not volunteer orchestration behavior unprompted** in otherwise ordinary EVALUATE/APPLY output.
...
**Justification:** Hard Constraints — **No breaking changes**: *"nothing that invalidates a cross-reference, listing number, or claim elsewhere in the chapter."* APPLY step 5: self-check confirms no cross-reference was invalidated before returning. **Two defensible answers (Pass 2 review):** Either path (a) narrower fix preserving cross-reference consistency or (b) `input_errors` without shipping a broken chapter is correct; the Pass 3 rubric must accept either via OR-logic, not force one canonical path.
...
**Justification:** **No section dilution**: *"no protected callout, core teaching passage, or unique example loses its content. Trimming restated prose… is allowed; removing content that exists nowhere else is not."* APPLY step 5 self-check includes `no_protected_passage_diluted`. **Two defensible answers (Pass 2 review):** Either path — minimal clause-level fix within the callout or `input_errors` without diluting unique content — is correct; the Pass 3 rubric must accept either via OR-logic, not force one canonical path.
...
**Justification:** EVALUATE step 2, first bullet: *"Structural break? (should already be 'no' if it passed step 1 — flag if this disagrees with step 1's filter, that's worth surfacing)."* **Two defensible answers (Pass 2 review):** After surfacing the disagreement, either `DO_NOT_APPLY` (subsection-scale paste) or `APPLY_WITH_CARE` (with explicit structural flag and a truly minimal subset) is acceptable; the non-negotiable criterion is surfacing the step-1 vs. checklist mismatch. The Pass 3 rubric must accept either final recommendation via OR-logic once the disagreement is explicitly stated.
